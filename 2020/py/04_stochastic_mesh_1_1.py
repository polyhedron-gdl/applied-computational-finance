...
for n in range(10000, 10002, 2):
    ...
    # HIGH ESTIMATOR -----------------------------------------------------------------------------
    #
    # First round with b/2 random variables
    #
    # Time t2 = T - 1
    #
    # calculating mesh
    #
    cont_2     = BS.BlackScholes('C', S21, K, r, delta, sigma, 1./3.) * math.exp(-r*(2./3.))
    exer_2     = np.maximum(S21 - K, 0)  * math.exp(-r*(2./3.))
    est_high_2 = np.maximum(exer_2, cont_2)
    #
    # calculating densities
    #
    density = np.zeros((int(b/2), int(b/2), n))
    for i in range(0, int(b/2)):
        for j in range(0, int(b/2)):
            c    = 1.0/(sigma * math.sqrt(dt) * S21[i,:])
            arg  = (np.log(S21[i,:]/S11[j,:]) - drift)/(sigma * math.sqrt(dt))
            density[j,i,:] = c * norm.pdf(arg)
    #
    # calculating weights
    #
    weights = np.zeros((int(b/2), int(b/2), n))
    for i in range(0, int(b/2)):
        for j in range(0, int(b/2)):
            somma = np.divide(np.sum(density[:,j,:], axis=0), b/2)
            weights[i, j,:] = np.divide(density[i, j,:], somma)
    #
    # Time t1 = T - 2
    #
    #
    est_high_1      = np.zeros((int(b/2),n))
    for i in range(0, int(b/2)):
        exer_1          = np.maximum(S11[i,:] - K, 0)  * math.exp(-r*dt)
        cont_1          = np.divide(np.sum(weights[i,:,:] * est_high_2 , axis=0), (b/2))
        est_high_1[i,:] = np.maximum(exer_1, cont_1)
    #
    # Time T = 0
    #
    cont_3          = np.divide(np.sum(est_high_1, axis=0), (b/2))
    est_high_FIRST  = np.maximum(S0 - K, cont_3)
    #
    # Secondo round with antithetic variables
    #
    #
    # Time t2 = T - 1
    #
    # calculating mesh
    #
    cont_2     = BS.BlackScholes('C', S22, K, r, delta, sigma, 1./3.) * math.exp(-r*(2./3.))
    exer_2     = np.maximum(S22 - K, 0)  * math.exp(-r*(2./3.))
    est_high_2 = np.maximum(exer_2, cont_2)
    #
    # calculating densities
    #
    ...
    for i in range(0, int(b/2)):
        for j in range(0, int(b/2)):
            c    = 1.0/(sigma * math.sqrt(dt) * S22[i,:])
            arg  = (np.log(S22[i,:]/S12[j,:]) - drift)/(sigma * math.sqrt(dt))
            density[j,i,:] = c * norm.pdf(arg)
    #
    # calculating weights
    #
    ...
    est_high_1      = np.zeros((int(b/2),n))
    for i in range(0, int(b/2)):
        exer_1          = np.maximum(S12[i,:] - K, 0)  * math.exp(-r*dt)
        cont_1          = np.divide(np.sum(weights[i,:,:] * est_high_2 , axis=0), b/2)
        est_high_1[i,:] = np.maximum(exer_1, cont_1)
    #
    # Time T = 0
    #
    cont_3          = np.divide(np.sum(est_high_1, axis=0), b/2)
    est_high_SECOND = np.maximum(S0 - K, cont_3)
    #
    est_high = 0.5 * est_high_FIRST + 0.5 * est_high_SECOND
    #
    # LOW ESTIMATOR ------------------------------------------------------------------------------
    #
    #
